    ...
    $(function() {
        ...
        $('.import_btn').click(function(){
            ...
            var url = "exportRoutingAnalysisDataForList?areaId="+searchBox.find('select[name="areaId"]').val() +
    ...
    function selectAll(){
        ...
        proUntil.request("routingAnalysisDataForList",paramObj,function(data){
            if("n" == data){
                $('.table_list').hide();
                $('.area_model').hide();
    ...
    function tableListClikc(){
        $('.table_list').find('table').find('tr').not(':first').click(function(){
            ...
            $.ajax({
                type : "post",
                url : "routingAnalysisDataForChart",
    ...
    function routingDataChart(result){
        myChart = echarts.init(document.getElementById('areaPower'));
        option = {
            title : {
